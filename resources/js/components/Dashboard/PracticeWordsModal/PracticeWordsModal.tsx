...
const PracticeWordsModal = ({
    config,
    user,
    setShowPracticeWordsModal,
    handleRemoveWord
}) => {
    const [drawnWord, setDrawnWord] = React.useState<IDrawnWord | undefined>(
        undefined
    );
    ...
    const getDrawnWord = async () => {
        await handleGetRequest(
            `${config?.paths?.API_URL}/words/random/new/1/${user?.id}`,
            user?.token
        ).then(res => {
            setWordAnswerStatus("");
    ...
    const getRandomWords = async () => {
        await handleGetRequest(
            `${config?.paths?.API_URL}/words/random/new/3/${user?.id}`,
            user?.token
        ).then((res: any) => {
    ...
    const checkWordSelection = async (selectedTranslation: string) => {
        if (!blockSelect) {
            await handlePostRequest(
                `${config?.paths?.API_URL}/words/check`,
                {
                    wordId: drawnWord?.id,
...
export default connect(mapStateToProps, {})(PracticeWordsModal);
