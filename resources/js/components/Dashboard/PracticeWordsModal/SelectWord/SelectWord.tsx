...
const SelectWord = ({ drawnWord, randomWordsList, checkWordSelection, wordAnswerStatus, showWordTranslation }) => {
    ...
    return (
        <>
            <p className="select-word__translation-header">{drawnWord && drawnWord.en && drawnWord.en.toLowerCase()} {showWordTranslation && ` - ${drawnWord && drawnWord.pl && drawnWord.pl.toLowerCase()}`}</p>

            <div className="select-word__options">
                {allWordsList && allWordsList.length > 0 && allWordsList.map((singleOption, i) => {
                    return <SingleOption
            ...
            {drawnWord && drawnWord.illustration &&
                <button className="red-btn box-shadow" onClick={handleShowIllustration}>
                    Pokaż ilustrację
                </button>
            }

            {showIllustration &&
                <img src={drawnWord.illustration.base64_image} className="select-word__illustration" />
            }
        </>
    )
}
