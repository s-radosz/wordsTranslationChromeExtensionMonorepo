...
const Statistics = ({ user }: userType) => {
  return (
    <div className='dashboard-statistics'>
      <SingleStat
        number={user?.countSavedWordsOverall ? user?.countSavedWordsOverall : 0}
        text='zapisanych ogółem'
      />

      <SingleStat
        number={user?.countSavedWordsLastWeek ? user?.countSavedWordsLastWeek : 0}
        text='w tym tygodniu'
      />

      <SingleStat
        number={user?.countSavedWordsToday ? user?.countSavedWordsToday : 0}
