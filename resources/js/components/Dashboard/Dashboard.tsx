...
const Dashboard = ({
    ...
    const [showIllustrationModal, setShowIllustrationModal] = React.useState(
        false
    );
    const [showPracticeWordsModal, setShowPracticeWordsModal] = React.useState(
        false
    );
    const [
        currentWordIdIllustration,
        setCurrentWordIdIllustration
    ] = React.useState(0);
    ...
    const handlePageClick = async (pageData: { selected: number }) => {
        let wordsResult = await handleGetRequest(
            `${config?.paths?.API_URL}/words/all/${
                user?.id
            }?page=${pageData?.selected + 1}`,
            user?.token
        );
        createWords(wordsResult);
    };
    ...
    return (
        <div className="dashboard">
            ...
            <PracticeWordsSection
                ...
                allowPracticeWords={
                    words?.result?.data?.length > 0 ? true : false
                }
            />
...
const mapDispatchToProps = dispatch => ({
    ...
    updateUserWordsCounts: payload =>
        dispatch(USERACTIONS.updateUserWordsCounts(payload))
});
