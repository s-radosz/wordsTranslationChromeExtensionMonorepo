...
const IllustrationModal = ({
  handleShowAlert,
  setShowIllustrationModal,
  currentWordIdIllustration,
  config,
  user,
}) => {
  ...
  const handleTextChange = (direction: string) => {
    ...
    if (direction === 'left') {
      ...
      //x, y, max-width
      for (var i = 0; i < lines.length; i++) ctx.fillText(lines[i], 30, 100 + i * lineheight)
    } else {
      let lines = handleFormatCanvasText(rightPersonText).split('\n')
      //x, y, max-width
      for (var i = 0; i < lines.length; i++) ctx.fillText(lines[i], 290, 100 + i * lineheight)
    }
  }
  ...
  const handleSaveIllustration = async () => {
    ...
    await handlePostRequest(
      `${config.paths.API_URL}/words/illustartion/new`,
      {
  ...
  const loadSavedIllustration = async () => {
    await handlePostRequest(
      `${config.paths.API_URL}/words/illustartion/find`,
      {
        wordId: currentWordIdIllustration,
      },
      user.token,
    ).then((res: { base64_image: string }) => {
      if (res.base64_image) {
        const ctx = canvasImage.current.getContext('2d')
        var imageObj1 = new Image()
        ...
        imageObj1.onload = function () {
          ctx.drawImage(imageObj1, 0, 0, 400, 300)
        }
      } else {
        handleAddEmptyImageToCanvas()
      }
...
export default connect(mapStateToProps, {})(IllustrationModal)
